...
func nodeToAST(n nodes.Node, t reflect.Type) reflect.Value {
	// switch to Obj Arr etc.
	// Obj has @type that you will use as map key
	switch o := n.(type) {
	case nil:
	...
	case nodes.Array:
		...
		for i, n := range o {
			...
			// in the case of slice of interface implementations, that contains non-pointer implementation that implements interface with pointer receiver o_0
...
func ValuetoNode(v reflect.Value, fs *token.FileSet) (nodes.Node, error) {
	if !v.IsValid() {
		return nil, nil
	}
	t := v.Type()
	...
	case reflect.Slice:
		...
		for i := 0; i < v.Len(); i++ {
			el, err := ValuetoNode(v.Index(i), fs)
			if err != nil {
				return nil, err
			}
			arr = append(arr, el)
